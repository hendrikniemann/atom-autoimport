...
function toImportString(base, toImport) {
...
export const nodeNativeModulePlugin = (rootDir, file, identifier, localFiles, installedPackages) => {
  // prettier-ignore
  const nativeModules = ['async_hooks', 'assert', 'buffer', 'child_process', 'console', 'constants', 'crypto', 'cluster', 'dgram', 'dns', 'domain', 'events', 'fs', 'http', 'http2', 'https', 'inspector', 'module', 'net', 'os', 'path', 'perf_hooks', 'process', 'punycode', 'querystring', 'readline', 'repl', 'stream', 'string_decoder', 'sys', 'timers', 'tls', 'tty', 'url', 'util', 'v8', 'vm', 'zlib'];
...
export const flowNamedImportPlugin = (rootDir, file, identifier, localFiles, installedPackages) => {
  const importables = [...localFiles.map(f => toImportString(file, f)), ...installedPackages];
  const imports = importables.map(source => ({
    type: 'named',
    identifier,
    source
  }));
  const importLines = importables.map((pckg, index) => `import { ${identifier} as x${index}, asdfasdf as z${index} } from '${pckg}';`);
  return flowCheckContents(rootDir, file, importLines.join('\n')).then(({
    errors
  }) => {
    const toLine = error => error.message[0].line;

    const importTypeError = errors.find(error => error.message[0].descr === `Cannot import the type \`${identifier}\` as a value. Use \`import type\` instead.`);
    ...
    const provokedErrors = errors.filter(error => error.message[0].descr.startsWith('Cannot import `asdfasdf` because there is no `asdfasdf` export in')).map(toLine);
    const failedLines = errors.filter(error => !error.message[0].descr.startsWith('Cannot import `asdfasdf` because there is no `asdfasdf` export in')).map(toLine);
...
export const installedDependencyDefaultImportPlugin = (rootDir, file, identifier, localFiles, installedPackages) => {
...
export const projectFileDefaultImportPlugin = (rootDir, file, identifier, localFiles, installedPackages) => {
